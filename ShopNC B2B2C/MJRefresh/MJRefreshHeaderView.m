...
- (void)layoutSubviews
{
    ...
    CGFloat statusX = self.mj_width * 0.5 - 30;
    CGFloat statusY = 0;
    CGFloat statusHeight = self.mj_height * 0.5;
    CGFloat statusWidth = self.mj_width * 0.5;
    // 1.状态标签
    self.statusLabel.frame = CGRectMake(statusX, statusY, statusWidth, statusHeight);
    
    // 2.时间标签
    CGFloat lastUpdateY = statusHeight;
    CGFloat lastUpdateX = self.mj_width * 0.5 - 30;
    CGFloat lastUpdateHeight = statusHeight-20;
    CGFloat lastUpdateWidth = self.mj_width * 0.5;
...
- (void)updateTimeLabel
{
    ...
    // 3.显示日期
    self.lastUpdateTimeLabel.text = [NSString stringWithFormat:@"上次刷新 %@", time];
}
