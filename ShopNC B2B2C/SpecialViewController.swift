...
class SpecialViewController: UITableViewController {
    ...
    /**调取用户数据**/
    func loadData() {
        let url = NSURL(string: API_URL + "index.php?act=index&op=special&special_id=" + sp_id)
        if url == nil {
        ...
        if let datas = datas_array as? NSArray {
            for val in datas {
                ...
                if homedata.objectForKey("home3") != nil {
                    let title = homedata.objectForKey("home3")!.objectForKey("title") as! String
                    if title != "" {
                        let newcell = Ordercellmodel()
                        newcell.type = "gap"
                        newcell.data = title
                ...
                if homedata.objectForKey("goods") != nil {
                    let title = homedata.objectForKey("goods")!.objectForKey("title") as! String
                    if title != "" {
                        let newcell = Ordercellmodel()
                        newcell.type = "gap"
                        newcell.data = title
